...
const DeviceDetails = () => {
  ...
  // This useEffect fetches all basic device info at once
  // IMPORTANT: Do NOT set deviceName here anymore; it now comes from /api/devices
  useEffect(() => {
    fetch(`${process.env.REACT_APP_EP}/data/devices/${id}/info`)
      .then(response => {
        if (!response.ok) {
      ...
      .then(apiResponse => {
        if (apiResponse.status === 'success' && apiResponse.data) {
          const { owner_name, phone_number, email_id, location } = apiResponse.data;

          // Device name is handled by the new effect below.
          setDeviceInfo({
            owner_name: owner_name || 'N/A',
            phone_number: phone_number || 'N/A',
            email_id: email_id || 'N/A',
      ...
      .catch(error => {
        console.error('Error fetching device details:', error);
        // Leave deviceName to be handled by the /api/devices effect
        setDeviceName('Error');
        setDeviceInfo({
          owner_name: 'Error',
          phone_number: 'Error',
  ...
  // NEW: Fetch device list and set deviceName (and implicit id confirmation) from /api/devices
  useEffect(() => {
    let cancelled = false;
    ...
    fetch(`${process.env.REACT_APP_EP}/api/devices`)
      ...
      .then(payload => {
        // DeviceDashboard maps (data.value || []), so mirror that shape handling.
        const list = Array.isArray(payload) ? payload : (payload.value || []);
        const dev = (list || []).find(d => String(d.id) === String(id)); // use Array.find

        if (!cancelled) {
          if (dev) {
            setDeviceName(dev.displayName || dev.name || 'N/A');
          } else {
            setDeviceName('N/A');
          }
        }
    ...
    return () => {
      cancelled = true; // guard against setState after unmount/reroute
    };
  }, [id]);
        ...
        .then(data => {
          ...
          setDeviceData({
            ...
            oxygenGenerator: {
              flowRate: data.oxygenGenerator.flowRate,
              pressure: data.oxygenGenerator.pressure,
              waterTemperature: data.oxygenGenerator.systemTemperature,
              systemTemperature: data.oxygenGenerator.systemTemperature,
              totalWaterOutlet: data.oxygenGenerator.totalWaterOutlet,
              timestamp: data.oxygenGenerator.timestamp
            }
