...
const baseCards = [
  {
    ...
    title: "Installed Devices",
    icon: svg1,
    roles: [Role.MasterAdmin, Role.CompanyAdmin, Role.CustomerAdmin, Role.Operator],
    route: "/devices",
  },
  {
    key: "registeredDevices",
...
const HomeDashboard = () => {
  ...
  const userRole = storedUser?.role !== undefined
    ? parseInt(storedUser.role, 10)
    : null;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Build URLs with caller info for tenant-scoped endpoints
        ...
        const [
          ...
          fetchJson(`${EP}/api/devices`),
          fetchJson(`${EP}/data/getprdevices`),
          fetchJson(customersUrl),
          fetchJson(`${EP}/data/companyassociates`),
          fetchJson(vendorsUrl),
        ]);
        ...
        setCounts({
          ...
          registeredDevices: getCountFromPayload(unwrap(registeredRes)),
          operators:         getCountFromPayload(unwrap(operatorsRes)),
          companyAdmins:     getCountFromPayload(unwrap(companyAdminsRes)),
          customerAdmins:    getCountFromPayload(unwrap(customerAdminsRes)),
        });
      } catch (e) {
        if (e.name !== "AbortError") console.error("Failed to load dashboard counts:", e);
      } finally {
        setCountsLoading(false);
      }
    })();
  ...
  const cards = useMemo(
    () => baseCards.map((c) => ({ ...c, value: counts[c.key] ?? 0 })),
    [counts]
  );
          ...
          <div className="statss-cards">
            {filteredCards.map((card, index) => (
              <Link
                key={card.key}
                to={card.route}
                className={`stat-card-link ${index === currentCardIndex ? "active" : "hidden-card"}`}
              >
                <div className="stat-card">
                  <div className="progress-circle">
